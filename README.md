...
### Lightweight Cloud Entry

This repository also includes a simple top-level `app.py` entrypoint:

```bash
python app.py
```

It starts the Gradio workbench with:

- `expected_data_lake_files=[]` to avoid forcing the large datalake download on first boot
- `server_name="0.0.0.0"` for container or hosted environments

For lightweight hosted deployments, install dependencies with:
